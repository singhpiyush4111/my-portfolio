 ...
 * Mobile responsive fix: separate mobile/desktop layouts
 * Creative name on mobile: "Piyush" solid + "Singh" outline stroke
 */
    ...
    >
      ...
      <div className="max-w-7xl mx-auto w-full">
        {/* ════════════════════════════════
            MOBILE LAYOUT  (< lg)
        ════════════════════════════════ */}
        <div className="flex flex-col lg:hidden min-h-[calc(100vh-80px)] justify-center gap-5 py-8">
          {/* ── Photo + floating cards  (TOP — matches screenshot) ── */}
          <div className="relative flex justify-center mb-2">
            <div ref={photoRef} className="relative">
              {/* Dashed ring */}
              <div className="absolute -inset-2 rounded-[26px] border-2 border-dashed border-[hsl(var(--accent))]/20 pointer-events-none" />

              {/* Photo */}
              <div className="relative w-[220px] h-[270px] rounded-[22px] overflow-hidden border border-border/40 shadow-xl">
                <img
                  src="/IMG-20260211-WA0000.jpg"
                  alt="Piyush Singh"
                  className="w-full h-full object-cover object-top brightness-105 contrast-110 saturate-105"
                  onError={(e) => {
                    const target = e.target as HTMLImageElement;
                    target.style.display = "none";
                    const parent = target.parentElement!;
                    parent.innerHTML = `<div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;background:hsl(var(--muted))"><div style="width:70px;height:70px;border-radius:50%;background:hsl(var(--accent)/0.15);display:flex;align-items:center;justify-content:center;font-size:1.6rem;font-weight:900;color:hsl(var(--accent))">PS</div></div>`;
                  }}
                />
              ...
              <div
                ref={card1Ref}
                className="absolute -top-3 -right-6 bg-background/95 backdrop-blur-sm border border-border/60 rounded-xl px-3 py-2 shadow-lg"
              >
                ...
                <p className="text-[10px] text-[hsl(var(--accent))] font-semibold mt-0.5">
                  Across 3 domains
                </p>
              </div>
              ...
              <div
                ref={card2Ref}
                className="absolute -bottom-3 -left-6 bg-background/95 backdrop-blur-sm border border-border/60 rounded-xl px-3 py-2 shadow-lg"
              >
                <div className="flex items-center gap-1.5 mb-0.5">
                  <Code2 size={10} className="text-[hsl(var(--accent))]" />
                  <span className="text-[10px] text-muted-foreground font-medium">
                    Experience
                  </span>
                </div>
              ...
              <div
                ref={pillRef}
                className="absolute top-1/2 -translate-y-1/2 -right-4 bg-[hsl(var(--accent))] text-white rounded-xl px-2 py-1.5 shadow-lg shadow-[hsl(var(--accent))]/35 flex flex-col items-center gap-1"
              >
                <Zap size={11} className="text-white/90" />
                <span
          ...
          {/* ── Badge (BOTTOM section — below photo) ── */}
          ...
          {/* Name — same as desktop, scramble works */}
          <h1
            ref={nameRef}
            className="text-5xl font-black text-foreground tracking-tight leading-[1.05] font-mono"
          >
            {FINAL_NAME}
          </h1>
          ...
          >
            Architecting high-performance backend systems for{" "}
          ...
          <div ref={ctaRef} className="flex gap-3">
            <button
              ...
              className="flex-1 bg-[hsl(var(--accent))] hover:bg-[hsl(var(--accent))]/90 text-white px-4 py-3.5 text-sm font-semibold rounded-xl transition-colors shadow-lg shadow-[hsl(var(--accent))]/25 cursor-pointer"
            >
              View Projects
            </button>
            <button
              ...
              className="flex-1 border-2 border-foreground/15 hover:border-[hsl(var(--accent))] hover:text-[hsl(var(--accent))] px-4 py-3.5 text-sm font-semibold rounded-xl transition-colors cursor-pointer bg-transparent"
            >
              Contact Me
            </button>
          </div>
          ...
          <div ref={socialRef} className="flex items-center gap-2">
            {[
              ...
              <a
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-muted hover:bg-[hsl(var(--accent))]/10 text-muted-foreground hover:text-[hsl(var(--accent))] transition-all text-xs font-medium"
                aria-label={label}
              >
                {icon}
                <span>{label}</span>
        ...
        <div className="hidden lg:grid grid-cols-2 gap-12 items-center min-h-[calc(100vh-80px)]">
          ...
          <div className="flex flex-col justify-center">
            ...
            <h1
              ref={nameRef}
              className="text-6xl md:text-7xl font-black text-foreground tracking-tight leading-[1.05] mb-5 font-mono"
            >
              {FINAL_NAME}
            </h1>
